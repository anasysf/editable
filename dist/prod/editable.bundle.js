...
var ___EXPOSE_LOADER_IMPORT___ = __webpack_require__(692);
var ___EXPOSE_LOADER_GET_GLOBAL_THIS___ = __webpack_require__(921);
...
class EditorManager {
    ...
    generateEditorHTML(defaultValue) {
    ...
    generateTextAreaHTML(defaultValue = '') {
    ...
    generateTextInputHTML(defaultValue = '') {
        const fragment = document.createDocumentFragment();
    ...
    generateNumberInputHTML(defaultValue = '') {
        const fragment = document.createDocumentFragment();
        const input = document.createElement('input');
        input.type = 'string';
        input.value = defaultValue;
    ...
    generateEmailInputHTML(defaultValue = '') {
        const fragment = document.createDocumentFragment();
        const input = document.createElement('input');
        input.type = 'email';
        input.value = defaultValue;
...
class FieldManager {
    _field;
    _rowIdx;
    static _iconSrcMap = new Map();
    _iconSrc = 'fa';
    constructor(field, rowIdx, iconSrc = 'fa') {
        this._field = field;
        this._rowIdx = rowIdx;
    ...
    static toggleIcon(target, id, title, rowIdx, className) {
        const fragment = document.createDocumentFragment();
        ...
        if (target instanceof HTMLSpanElement) {
            ...
            if (!span)
                throw new ReferenceError('Could not find a <span> element.');
            span.replaceWith(fragment);
        }
    }
    static generateIconWrapperBtn(id, title, rowIdx) {
        const iconWrapper = document.createElement('span');
    ...
    static generateIconBtn(id, title, rowIdx, className) {
        const icon = document.createElement('i');
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const fieldManager_1 = __importDefault(__webpack_require__(20));
class Column {
    _dataOptions;
    _editableOptions;
    constructor(dataOptionsStr, editableOptions) {
    ...
    set dataOptions(dataOptionsStr) {
    ...
    validateDataOptions(dataOptions) {
        return (typeof dataOptions !== 'undefined' &&
            dataOptions !== null &&
    ...
    parseDataOptionsStr(dataOptionsStr) {
        if (!dataOptionsStr)
            throw new TypeError('Expected a [data-options] attribute instead received undefined.');
        return JSON.parse(dataOptionsStr);
    }
    generateConfigColumns() {
        switch (this.field) {
            case 'checkbox':
                return {
                    data: null,
                    render: (_data, _type, _row, { row }) => {
                        const fieldManager = new fieldManager_1.default(this.field, row, this.iconSrc);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const editorManager_1 = __importDefault(__webpack_require__(988));
const fieldManager_1 = __importDefault(__webpack_require__(20));
const http_1 = __importDefault(__webpack_require__(188));
const responseError_1 = __importDefault(__webpack_require__(536));
class EventHandler {
    _columns = new Map();
    _dataTable;
    _editableOptions;
    constructor(columns, dataTable, editableOptions) {
    ...
    get deleteDataSrcMethod() {
        return typeof this.editableOptions.deleteDataSrc === 'object'
            ? this.editableOptions.deleteDataSrc.method
            : 'POST';
    }
    get deleteDataSrcFormat() {
        return typeof this.editableOptions.deleteDataSrc === 'object'
    ...
    get postDataSrcMethod() {
        return typeof this.editableOptions.postDataSrc === 'object'
            ? this.editableOptions.postDataSrc.method
            : 'POST';
    }
    get postDataSrcFormat() {
        return typeof this.editableOptions.postDataSrc === 'object'
    ...
    handle(evt) {
        ...
        switch (targetName) {
            ...
            case 'delete-row-icon': {
                return void this.handleOnDeleteRowClick(target);
            }
            ...
            case 'save-row-edit-icon': {
                return void this.handleOnSaveEditRowClick(target);
            }
            case 'save-new-row-icon': {
                return void this.handleOnSaveNewRowClick(target);
            }
    ...
    handleOnEditRowClick(target) {
        const hasEditor = [...this.columns.values()].some((column) => column.editorOptions);
        if (!hasEditor)
            return;
        const tr = target.closest('tr');
        if (!tr)
            throw new ReferenceError(`Could not find the closest row.`);
        ...
        [...tds].forEach((td, idx) => {
            ...
            if (!column)
                throw new ReferenceError(`Could not find a column at index: ${idx}.`);
            const editorOptions = column.editorOptions;
            if (!editorOptions)
                return;
            const editorManager = new editorManager_1.default(editorOptions);
            const field = column.field;
            const editor = editorManager.generateEditorHTML(rowData[field]);
        ...
        fieldManager_1.default.toggleIcon(target, 'save-row-edit', 'Enregistrer', rowIdx, editIcon);
        const deleteIcon = iconSrcMap['cancel-row-edit'];
        fieldManager_1.default.toggleIcon(deleteIconBtn, 'cancel-row-edit', 'Annuler', rowIdx, deleteIcon);
    }
    handleOnCancelEditRowClick(target) {
        const tr = target.closest('tr');
        if (!tr)
            throw new ReferenceError(`Could not find the closest row.`);
        const row = this.dataTable.row(tr);
    ...
    async handleOnSaveEditRowClick(target) {
        const tr = target.closest('tr');
        if (!tr)
            throw new ReferenceError(`Could not find the closest row.`);
        const tds = tr.cells;
    ...
    async handleOnDeleteRowClick(target) {
        const tr = target.closest('tr');
        if (!tr)
            throw new ReferenceError(`Could not find the closest row.`);
        ...
        if (tds.length === 0)
            throw new ReferenceError(`Could not find a single cell on this row.`);
        const row = this.dataTable.row(tr);
        const http = new http_1.default(this.deleteDataSrcURL);
        const formData = {
            [rowIdKey]: row.id(),
        };
        try {
            await http.send({ method: this.deleteDataSrcMethod }, this.deleteDataSrcFormat, formData);
    ...
    async handleOnSaveNewRowClick(target) {
        const tr = target.closest('tr');
        if (!tr)
            throw new ReferenceError(`Could not find the closest row.`);
        const rowIdKey = this.editableOptions.rowId;
    ...
    handleOnCancelNewRowClick(target) {
        const tr = target.closest('tr');
        if (!tr)
            throw new ReferenceError(`Could not find the closest row.`);
        tr.remove();
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const datatables_net_bs5_1 = __importDefault(__webpack_require__(950));
const column_1 = __importDefault(__webpack_require__(846));
const eventHandler_1 = __importDefault(__webpack_require__(640));
const editorManager_1 = __importDefault(__webpack_require__(988));
const fieldManager_1 = __importDefault(__webpack_require__(20));
/**
...
class Editable {
    ...
    constructor(table, options) {
        ...
        this._eventHandler = new eventHandler_1.default(this.columns, this.dataTable, this.options);
        this.registerEvents();
    }
    get table() {
        return this._table;
    ...
    set options(options) {
        ...
        if (!rowId)
            throw new ReferenceError('A rowId must be set.');
        this._options = {
            rowId,
            dataSrc,
            updateDataSrc,
    ...
    set dataTable(config) {
        this._dataTable = new datatables_net_bs5_1.default(this.table, config);
    }
    set columns(table) {
        const colgroup = this.getColgroup(table);
        const cols = this.getColsInColgroup(colgroup);
        this._columns = new Map(cols.map((col, idx) => [idx, new column_1.default(col.dataset.options, this.options)]));
    }
    getColgroup(table = this.table) {
        const colgroups = table.getElementsByTagName('colgroup');
        if (colgroups.length === 0)
            throw new ReferenceError(`The <table> element with the id: #${table.id} must contain a <colgroup>.`);
        const colgroup = colgroups.item(0);
        if (!colgroup)
            throw new ReferenceError(`The <table> element with the id: #${table.id} must contain a <colgroup>.`);
        return colgroup;
    }
    getColsInColgroup(colgroup) {
        if (!colgroup.hasChildNodes())
            throw new ReferenceError('The <colgroup> element must contain at least one <col /> element.');
    ...
    setDataTableConfig() {
        const columns = [...this.columns.values()].map((column) => column.generateConfigColumns());
        const rowId = this.options.rowId;
        return {
            ajax: {
                url: this.dataSrcURL,
                dataSrc: this.dataSrcProp,
                method: this.dataSrcMethod,
    ...
    addRow() {
        ...
        fieldManager_1.default.toggleIcon(newRowEditIcon, 'save-new-row', 'Enregistrer', rowIdx, saveRowIcon);
        const cancelRowIcon = iconSrcMap['cancel-new-row'];
        fieldManager_1.default.toggleIcon(newRowDeleteIcon, 'cancel-new-row', 'Annuler', rowIdx, cancelRowIcon);
        if (!currentPageRows) {
            newRow.draw(false);
        }
        else {
    ...
    registerEvents() {
        if (!this.tbody)
            throw new ReferenceError(`Could not find a <tbody> element on this <table>: #${this.table.id}.`);
...
class HTTP {
    ...
    async send(init, format = 'json', body) {
        init = {
            ...init,
            headers: {
                'Content-Type': format === 'json' ? 'application/json' : 'multipart/form-data',
            },
            body: format === 'json' && body !== undefined
                ? JSON.stringify(body)
                : this.formDataFromObj(body),
        };
        const res = await fetch(this.URL, init);
        if (!res.ok)
            throw new responseError_1.default(res.status, res.statusText, res.url);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const editable_1 = __importDefault(__webpack_require__(89));
(($) => {
    return ($.fn.editable = function (options) {
        return new editable_1.default(this.get(0), options);
    });
})($);
...
__webpack_require__.d(__webpack_exports__, {
  "default": () => (/* binding */ dataTables_bootstrap5)
});

// EXTERNAL MODULE: ./node_modules/.pnpm/jquery@3.7.1/node_modules/jquery/dist/jquery-exposed.js
var jquery_exposed = __webpack_require__(197);
;// CONCATENATED MODULE: ./node_modules/.pnpm/datatables.net@1.13.8/node_modules/datatables.net/js/jquery.dataTables.mjs
/*! DataTables 1.13.7
...
;// CONCATENATED MODULE: ./node_modules/.pnpm/datatables.net-bs5@1.13.8/node_modules/datatables.net-bs5/js/dataTables.bootstrap5.mjs
...
let dataTables_bootstrap5_$ = jquery_exposed;
...
dataTables_bootstrap5_$.extend( true, jquery_dataTables.defaults, {
	dom:
...
dataTables_bootstrap5_$.extend( jquery_dataTables.ext.classes, {
...
jquery_dataTables.ext.renderer.pageButton.bootstrap = function ( settings, host, idx, buttons, page, pages ) {
	var api     = new jquery_dataTables.Api( settings );
	var classes = settings.oClasses;
	var lang    = settings.oLanguage.oPaginate;
	var aria = settings.oLanguage.oAria.paginate || {};
	var btnDisplay, btnClass;
	...
	var attach = function( container, buttons ) {
		...
		var clickHandler = function ( e ) {
			e.preventDefault();
			if ( !dataTables_bootstrap5_$(e.currentTarget).hasClass('disabled') && api.page() != e.data.action ) {
		...
		for ( i=0, ien=buttons.length ; i<ien ; i++ ) {
			...
			if ( Array.isArray( button ) ) {
				...
				if ( btnDisplay ) {
					...
					node = dataTables_bootstrap5_$('<li>', {
							'class': classes.sPageButton+' '+btnClass,
							'id': idx === 0 && typeof button === 'string' ?
								settings.sTableId +'_'+ button :
								null
						} )
						.append( dataTables_bootstrap5_$('<a>', {
								'href': disabled ? null : '#',
								'aria-controls': settings.sTableId,
								'aria-disabled': disabled ? 'true' : null,
								'aria-label': aria[ button ],
	...
	var hostEl = dataTables_bootstrap5_$(host);
	// IE9 throws an 'unknown error' if document.activeElement is used
	// inside an iframe or frame. 
	var activeEl;
...
/* harmony default export */ const dataTables_bootstrap5 = (jquery_dataTables);
...
/******/ 	var __webpack_exports__ = __webpack_require__(288);
/******/ 	
/******/ })()
;